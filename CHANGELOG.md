...
- **英文界面补齐**：此前有约 40 处用户可见文案没有走本地化，系统语言为英文时仍然显示中文——包括状态栏与菜单的状态消息（`ReleaseStore` 的更新检查、`LifecycleServices` 的 GitHub Releases 错误）、窗口标题与菜单标题（「所有 Xcode 版本」「Xcode Switcher 设置」「项目」）、Runtime 下载进度、磁盘清理目录名、签名页的空值占位（「未设置」「未命名 Target」）、以及环境体检的详情与建议（`EnvironmentDoctor`）。现在全部接入 String Catalog 并补上英文。

### 变更

- 签名页的「未设置」判定改为对同一个本地化值比较：原先 `value == "未设置"` 在英文环境下会因为显示值变成译文而失效。
- **新增本地化门禁**：`Scripts/audit_unlocalized_strings.py` 比较源码里的中文字面量与编译器写出的 `.stringsdata`，任何含中文却没进 String Catalog 的字面量都会让 CI 失败（可加 `unlocalized-audit:ok：<理由>` 豁免）。此前 `sync` 与 `verify` 只看已经进目录的字符串，发现不了「文案根本没进目录」——上一项英文补齐里那约 40 处就是这样漏了很久。
